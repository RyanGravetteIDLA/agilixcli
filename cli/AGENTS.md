# Agilix Buzz Printed CLI Agent Guide

This directory is a generated `agilix-buzz-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
agilix-buzz-pp-cli doctor --json
agilix-buzz-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
agilix-buzz-pp-cli which "<capability>" --json
agilix-buzz-pp-cli <command> --help
```
...
```bash
agilix-buzz-pp-cli <command> --agent
```
...
```bash
agilix-buzz-pp-cli <command> --help
agilix-buzz-pp-cli <command> --dry-run --agent
```

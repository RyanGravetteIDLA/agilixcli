...
type Config struct {
	...
	BuzzToken        string          `toml:"token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.agilixbuzz.com",
	}
	...
	// Env var overrides
	if v := os.Getenv("BUZZ_TOKEN"); v != "" {
		cfg.BuzzToken = v
		cfg.markEnvOverride("BuzzToken")
		cfg.AuthSource = "env:BUZZ_TOKEN"
		cfg.CredentialSource = "env:BUZZ_TOKEN"
	}
	...
	if v := os.Getenv("AGILIX_BUZZ_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("AGILIX_BUZZ_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "agilix-buzz-pp-cli", "config.toml"), nil
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.BuzzToken = ""
	delete(c.envOverrides, "BuzzToken")
	c.updateFileConfigField("BuzzToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["BuzzToken"] {
			out.BuzzToken = c.fileConfig.BuzzToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "BuzzToken":
		c.fileConfig.BuzzToken = c.BuzzToken
	}
}

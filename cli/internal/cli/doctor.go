...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  agilix-buzz-pp-cli doctor
  agilix-buzz-pp-cli doctor --json
  agilix-buzz-pp-cli doctor --fail-on warn
  agilix-buzz-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set it with: agilix-buzz-pp-cli auth set-token <token> or export BUZZ_TOKEN=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("BUZZ_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "BUZZ_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "BUZZ_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						// 'users get' with no args returns the current signed-on
						// user — a no-required-args, auth-dependent probe that is
						// the canonical Buzz token check. Prefer it over the
						// generic suggester, which may pick a command that needs
						// an entity id (e.g. 'announcements get').
						suggestion := "users get"
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "agilix-buzz-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'agilix-buzz-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'agilix-buzz-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'agilix-buzz-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'agilix-buzz-pp-cli sync' to refresh."
	}
	return report
}

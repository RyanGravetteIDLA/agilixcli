...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AGILIX_BUZZ_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AGILIX_BUZZ_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "agilix-buzz-pp-cli/feedback")
...
When ` + "`AGILIX_BUZZ_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AGILIX_BUZZ_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  agilix-buzz-pp-cli feedback list
  agilix-buzz-pp-cli feedback list --limit 5
  agilix-buzz-pp-cli feedback list --json`,

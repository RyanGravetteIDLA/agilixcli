...
func detectPartialFailure(data []byte) *partialFailureReport {
	...
	// DLAP batch shape: an outer response.code of OK can still wrap per-item
	// failures in responses.response[], each with its own code. Surface any
	// non-OK inner code as a partial failure so scripted callers get a non-zero
	// exit (respecting --allow-partial-failure). DLAP never sets partialFailureError.
	if dlap := detectDLAPPartialFailure(top); dlap != nil {
		return dlap
	}
...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'agilix-buzz-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token."+
			"\n      Set it with: agilix-buzz-pp-cli auth set-token <token> or export BUZZ_TOKEN=\"your-token-here\""+
			"\n      Run 'agilix-buzz-pp-cli doctor' to check auth status.", err))

...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  agilix-buzz-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  agilix-buzz-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"announcements", "announcements-cmd?cmd=getuserannouncementlist", "announcements-cmd?cmd=listrestorableannouncements", "assessments", "assessments-cmd?cmd=listrestorablequestions", "badges", "blogs", "courses", "discussions", "discussions-cmd?cmd=listrestorablemessages", "domains", "domains-cmd?cmd=getdomainparentlist", "enrollments", "enrollments-cmd?cmd=getenrollmentgrouplist", "enrollments-cmd?cmd=listenrollmentsbyteacher", "enrollments-cmd?cmd=listentityenrollments", "enrollments-cmd?cmd=listuserenrollments", "files", "files-cmd?cmd=listrestorabledocuments", "files-cmd?cmd=listrestorableresources", "gradebook", "gradebook-cmd?cmd=getgradebooklist", "groups", "groups-cmd?cmd=getgroupenrollmentlist", "items", "items-cmd?cmd=findpersonalizedentities", "items-cmd?cmd=getitemlist", "items-cmd?cmd=listassignableitems", "items-cmd?cmd=listrestorableitems", "objectives", "objectives-cmd?cmd=getobjectivemaplist", "objectives-cmd?cmd=getobjectivesubjectlist", "objectives-cmd?cmd=listobjectivesets", "peergrading", "reports", "reports-cmd?cmd=getrunnablereportlist", "rights", "rights-cmd?cmd=geteffectivesubscriptionlist", "rights-cmd?cmd=getentitysubscriptionlist", "rights-cmd?cmd=getrightslist", "rights-cmd?cmd=getsubscriptionlist", "system", "tokens", "users", "wikis", "wikis-cmd?cmd=listrestorablewikipages"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  agilix-buzz-pp-cli workflow status

  # Show status as JSON
  agilix-buzz-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("agilix-buzz-pp-cli")
			}

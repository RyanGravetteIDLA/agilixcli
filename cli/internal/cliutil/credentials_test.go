...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"AGILIX_BUZZ_CONFIG",
		"AGILIX_BUZZ_CONFIG_DIR",
		"AGILIX_BUZZ_DATA_DIR",
		"AGILIX_BUZZ_STATE_DIR",
		"AGILIX_BUZZ_CACHE_DIR",
		"AGILIX_BUZZ_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
	...
	return home, filepath.Join(home, ".config", "agilix-buzz-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "agilix-buzz-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("BUZZ_TOKEN", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "agilix-buzz-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "agilix-buzz-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "agilix-buzz-pp-cli", "credentials.toml")

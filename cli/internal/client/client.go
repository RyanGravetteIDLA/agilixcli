...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	rawContentType := ""
	if body != nil {
		if ru, ok := body.(RawUpload); ok {
			// Raw binary/text upload (e.g. PutResource): send the bytes as-is
			// with the caller's content type; do not JSON-marshal or DLAP-wrap.
			bodyBytes = ru.Data
			rawContentType = ru.ContentType
		} else {
			// DLAP create/update/put commands require their flat flag-built body to
			// be wrapped in a {"requests":{"<entity>":[...]}} (or single {"request":{}})
			// envelope. Wrap here centrally so every generated command's flag path
			// works; already-wrapped (--stdin) and query-param bodies pass through.
			body = wrapDLAPRequestBody(path, body)
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		req.Header.Set("User-Agent", "agilix-buzz-pp-cli/0.1 (+https://github.com/mvanhorn/printing-press-library)")
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "agilix-buzz-pp-cli/0.1.0")
		}
		...
		if resp.StatusCode < 400 {
			...
			// Proactive Buzz time-limit adaptation: if the provisioned-ms budget
			// is running low, pause briefly so the next call stays under the limit
			// instead of triggering a 429. No-op for non-Buzz / unthrottled responses.
			if d := buzzThrottleDelay(resp.Header); d > 0 {
				_ = sleepContext(ctx, d)
			}
			// DLAP returns HTTP 200 even for logical failures; the real status
			// is in response.code. Surface non-OK envelopes as APIErrors so the
			// CLI's exit-code mapping applies. OK/non-envelope bodies pass through.
			if derr := dlapError(method, c.displayURL(path, authHeader), clean); derr != nil {
				return nil, derr.StatusCode, derr
			}
			return json.RawMessage(clean), resp.StatusCode, nil
		}
		...
		// Server error - retry with backoff. Buzz 503 Server Busy responses
		// carry a Retry-After estimate; honor it when present (it reflects the
		// server's own load estimate) and fall back to exponential backoff
		// otherwise. Either way the wait grows across attempts.
		if resp.StatusCode >= 500 && attempt < maxRetries {
			wait := time.Duration(math.Pow(2, float64(attempt))) * time.Second
			if ra := cliutil.RetryAfter(resp); ra > wait {
				wait = ra
			}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export BUZZ_TOKEN=<your-token> or agilix-buzz-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.BuzzToken)
	}
